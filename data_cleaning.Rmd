...
health_dff <- select(health_dff, -c(disabled, marriage_st, employment, notcitizen, hispanic_t, white_t, black_t))
...
race$Largest_Column <- as.factor(race$Largest_Column)

#adding the race information to the data frame
race_list <- as.list(race$Largest_Column)
...
health_dff <- health_dff %>% 
  mutate(racial_majority = recode(racial_majority, white_pct = 'white', hispanic_pct = 'nonwhite', black_pct = 'nonwhite'))
```
...
```{r}
#creating the csv file
write.csv(health_dff_comp, "cleaned_health_data.csv", row.names = FALSE)
```
